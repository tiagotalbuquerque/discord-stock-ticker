...
func addInitialStock() *Stock {
	...
	extendActivity := env.GetBoolDefault("EXTEND_ACTIVITY", false)
	...
	case "":
		...
		stock = NewStock(ticker, token, stockName, nickname, color, decorator, frequency, currency, activity, extendActivity)
	default:
